...
def generate_fou(
    ...
    for i in range(1, n):
        # dX = -θX*dt + σ*dB_H
        fou_series[i] = (
            fou_series[i - 1] - theta * fou_series[i - 1] * dt + sigma * dB_H[i - 1]
        )
...
def add_contamination(
    ...
    if contamination_type == "none":
        ...
        for _ in range(n_pops):
            ...
            pop_amplitude = (
                np.random.choice([-1, 1]) * contamination_level * 10 * np.std(data)
            )
        ...
        for _ in range(n_motion_events):
            ...
            motion_amplitude = (
                np.random.choice([-1, 1]) * contamination_level * 3 * np.std(data)
            )
        ...
        powerline_noise = powerline_amplitude * np.sin(
            2 * np.pi * powerline_freq * t / 250.0
        )
        ...
        for _ in range(n_heavy_events):
            ...
            heavy_amplitude = (
                np.random.power(alpha, end_idx - start_idx)
                * contamination_level
                * 5
                * np.std(data)
            )
        ...
        for _ in range(n_avalanches):
            ...
            avalanche_size = (
                np.random.power(2.0) * contamination_level * 10 * np.std(data)
            )
            ...
            noise_component = (
                np.random.randn(end_idx - start_idx) * 0.1 * avalanche_size
            )
        ...
        for _ in range(n_spikes):
            ...
            spike_pattern = (
                spike_amplitude * np.exp(-t_spike / 3) * np.random.choice([-1, 1])
            )
        ...
        while i < n:
            ...
            contaminated[i:burst_end] += burst_amplitude * np.random.randn(
                burst_end - i
            )
...
def generate_grid(
    ...
    if biomedical_scenarios:
        try:
            from .biomedical_scenarios import (
                ...
                for length in lengths:
                    for contamination in contaminations:
                        for scenario in biomedical_scenarios:
                            current_seed = (
                                seed + sample_id if seed is not None else sample_id
                            )
            ...
            print(
                "Warning: Biomedical scenarios module not available, skipping biomedical data generation"
            )
            ...
            for contamination in contaminations:
                for generator_type in generators:
                    ...
                    if generator_type in generator_functions:
                        data = generator_functions[generator_type](
                            length, hurst, seed=current_seed
                        )
                    else:
                        # Fallback to fBm
                    ...
                    if contamination != "none":
                        data = add_contamination(
                            data, contamination, contamination_level, current_seed
                        )
