...
from .generation import TimeSeriesSample, fbm_davies_harte
        ...
        if 0 <= qrs_time <= t[-1]:
            ...
            if 0 <= qrs_idx < n:
                ...
                qrs_shape = np.exp(
                    -(
                        (
                            (np.arange(end_idx - start_idx) - qrs_width // 2)
                            / (qrs_width // 4)
                        )
                        ** 2
                    )
            ...
            if p_time >= 0:
                ...
                if 0 <= p_idx < n:
                    ...
                    p_shape = np.exp(
                        -(
                            (
                                (np.arange(end_idx - start_idx) - p_width // 2)
                                / (p_width // 3)
                            )
                            ** 2
                        )
            ...
            if t_time <= t[-1]:
                ...
                if 0 <= t_idx < n:
                    ...
                    t_shape = np.exp(
                        -(
                            (
                                (np.arange(end_idx - start_idx) - t_width // 2)
                                / (t_width // 3)
                            )
                            ** 2
                        )
...
def generate_respiratory_scenario(
    n: int,
    hurst: float,
    ...
    if contamination_level > 0:
        respiratory_signal = add_respiratory_artifacts(
            respiratory_signal, contamination_level, seed
        )
...
def add_eeg_artifacts(
    data: np.ndarray, contamination_level: float, seed: Optional[int] = None
) -> np.ndarray:
    """
    ...
    if np.random.random() < contamination_level:
        ...
        for _ in range(n_eye_movements):
            ...
            eye_artifact = np.linspace(
                0, np.random.choice([-1, 1]) * 200, end_idx - start_idx
            )
...
def add_ecg_artifacts(
    ...
    if np.random.random() < contamination_level * 0.4:
        ...
        for _ in range(n_motion_artifacts):
            ...
            motion_artifact = np.random.choice([-0.5, 0.5]) * np.ones(
                end_idx - start_idx
            )
...
def add_respiratory_artifacts(
    ...
    if np.random.random() < contamination_level * 0.3:
        ...
        for _ in range(n_coughs):
            ...
            cough_artifact = (
                np.exp(-np.arange(end_idx - idx) / 5) * np.random.choice([-1, 1]) * 2
            )
...
def generate_biomedical_scenario(
    scenario_type: str,
    n: int,
    hurst: float,
    contamination_level: float = 0.1,
