...
def create_accuracy_comparison_plot(
    results: List[BenchmarkResult], save_path: Optional[str] = None
) -> plt.Figure:
    """
...
def create_error_vs_hurst_plot(
    results: List[BenchmarkResult], save_path: Optional[str] = None
) -> plt.Figure:
    """
    ...
    for i, estimator in enumerate(estimators):
        ...
        for result in estimator_results:
            if (
                ...
                and result.absolute_error is not None
                and not np.isnan(result.absolute_error)
            ):
        ...
        if hurst_values and errors:
            ax.scatter(
                hurst_values, errors, label=estimator, alpha=0.6, color=colors[i], s=50
            )
...
def create_uncertainty_quantification_plot(
    results: List[BenchmarkResult], save_path: Optional[str] = None
) -> plt.Figure:
    """
    ...
    for result in results:
        estimator = result.estimator
        if result.confidence_interval is not None and result.true_hurst is not None:
            ...
            if result.standard_error is not None and not np.isnan(
    ...
    bars2 = ax2.bar(
        range(len(estimators)), coverage_rates, color="lightgreen", alpha=0.7
    )
    ...
    std_errors = [
        (
            np.mean(ci_data[est]["standard_errors"])
            if ci_data[est]["standard_errors"]
            else 0
        )
    ...
    ax4.axvline(
        x=np.mean(widths), color="blue", linestyle="--", alpha=0.7, label="Mean Width"
    )
    ax4.legend()
    ax4.grid(True, alpha=0.3)
...
def create_focused_analysis_report(
    results: List[BenchmarkResult], output_dir: str = "./benchmark_plots"
) -> None:
    """
