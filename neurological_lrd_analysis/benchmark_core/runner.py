...
from ..benchmark_registry.registry import EstimatorResult, get_registry
...
def run_benchmark_on_dataset(
    samples: List[TimeSeriesSample], config: BenchmarkConfig
    ...
    if config.estimators is not None:
        available_estimators = [
            est for est in registry if est.name in config.estimators
        ]
    else:
    ...
    for sample in samples:
        if config.verbose:
            print(
                f"Processing sample: H={sample.true_hurst}, "
        ...
        for estimator in sorted(estimators):
            ...
            if successful:
                estimates = [
                    r.hurst_estimate
                    for r in successful
                    if not np.isnan(r.hurst_estimate)
                ]
                biases = [
                    r.bias
                    for r in successful
                    if r.bias is not None and not np.isnan(r.bias)
                ]
                errors = [
                    r.absolute_error
                    for r in successful
                    if r.absolute_error is not None and not np.isnan(r.absolute_error)
                ]
                p_values = [
                    r.p_value
                    for r in successful
                ...
                if estimates:
                    ...
                    f.write(
                        f"  Mean computation time: {np.mean([r.computation_time for r in successful]):.4f}s\n"
                    )
                    ...
                    if biases:
                        ...
                        f.write(
                            f"  RMSE: {np.sqrt(np.mean([b**2 for b in biases])):.4f}\n"
                        )

                    if p_values:
                        significant = sum(1 for p in p_values if p < 0.05)
                        f.write(
                            f"  Significant estimates (p<0.05): {significant}/{len(p_values)} ({significant/len(p_values):.1%})\n"
                        )
                    ...
                    if confidence_intervals:
                        ...
                        f.write(
                            f"  Mean confidence interval: [{np.mean(ci_lowers):.4f}, {np.mean(ci_uppers):.4f}]\n"
                        )
...
def calculate_estimator_score(
    analysis: Dict[str, Any], estimator: str, weights: ScoringWeights
) -> float:
    """
...
def analyze_benchmark_results(results: List[BenchmarkResult]) -> Dict[str, Any]:
    ...
    for estimator in estimators:
        ...
        successful = [
            ...
            if r.convergence_flag and not np.isnan(r.hurst_estimate)
        ]
        ...
        biases = [
            r.bias for r in successful if r.bias is not None and not np.isnan(r.bias)
        ]
        errors = [
            r.absolute_error
            for r in successful
        ...
        relative_errors = [
            r.relative_error
            for r in successful
            if r.relative_error is not None and not np.isnan(r.relative_error)
        ]
        p_values = [
            r.p_value
            for r in successful
        ...
        standard_errors = [
            r.standard_error
            for r in successful
            if r.standard_error is not None and not np.isnan(r.standard_error)
        ]
...
def create_leaderboard(
    results: List[BenchmarkResult], weights: Optional[ScoringWeights] = None
) -> pd.DataFrame:
    """
    ...
    for estimator, metrics in analysis.items():
        ...
        ci_str = "N/A"
        if not np.isnan(metrics["mean_ci_lower"]) and not np.isnan(
            metrics["mean_ci_upper"]
        ):
        ...
        leaderboard_data.append(
            {
                ...
                "Mean Bias": (
                    ...
                    if not np.isnan(metrics["mean_bias"])
                    else "N/A"
                ),
                "Std Bias": (
                    f"{metrics['std_bias']:.4f}"
                    if not np.isnan(metrics["std_bias"])
                    else "N/A"
                ),
                "Mean Absolute Error": (
                    f"{metrics['mean_absolute_error']:.4f}"
                    if not np.isnan(metrics["mean_absolute_error"])
                    else "N/A"
                ),
                "RMSE": (
                    f"{metrics['rmse']:.4f}" if not np.isnan(metrics["rmse"]) else "N/A"
                ),
                "Mean Relative Error": (
                    f"{metrics['mean_relative_error']:.2%}"
                    if not np.isnan(metrics["mean_relative_error"])
                    else "N/A"
                ),
                "Mean Standard Error": (
                    f"{metrics['mean_standard_error']:.4f}"
                    if not np.isnan(metrics["mean_standard_error"])
                    else "N/A"
                ),
                "95% CI": ci_str,
                "Mean CI Width": (
                    f"{metrics['mean_ci_width']:.4f}"
                    if not np.isnan(metrics["mean_ci_width"])
                    else "N/A"
                ),
                "Significance Rate": (
                    f"{metrics['significance_rate']:.2%}"
                    if not np.isnan(metrics["significance_rate"])
                    else "N/A"
                ),
